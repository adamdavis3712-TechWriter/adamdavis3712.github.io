# Adam Davis - Senior Technical Writer

I'm a Senior Technical Writer who specializes in writing clear, user-friendly documentation for both internal and external audiences.

## Portfolio

### Tutorials
- [Getting Started with Docker](./tutorials/docker.md)

### Guides
- [How to Deploy a Website0(./guides/deploy.md)

### API Documentation
- [Sample API Docs](./api/sample-api.md)

## Tools I Write About
- Docker
- APIs
- JavaScript

## Contact
- Email: adamdavis3712@gmail.com
